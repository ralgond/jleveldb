...
public class DBImpl implements DB {
	...
	void compactMemTable() {
		...
		//System.out.println("[DEBUG] DBImpl.compactMemTable 1");
	...
	static class LogReporter implements LogReader.Reporter {
		...
		public void corruption(int bytes, Status s) {
		    Logger0.log0(infoLog, "{}{}: dropping {} bytes; {}",
	...
	Status recoverLogFile(final long logNumber, boolean lastLog, Boolean0 saveManifest,
		...
		
		{
			...
			env.getFileSize(fname, fileSize0);
			System.out.printf("[DEBUG] recoverLogFile, logFileName, name=%s, size=%d\n", fname, fileSize0.getValue());
		}
		...
		while (true) {
			boolean readRes = reader.readRecord(record, scratch);
			if (readRes && status.ok()) {
				
			} else {
				System.out.printf("[DEBUG] recoverLogFile 0, readRes=%s, status=%s\n", readRes, status);
				break;
			}
		    ...
		    if (mem.approximateMemoryUsage() > options.writeBufferSize) {
		    	System.out.println("[DEBUG] recoverLogFile 1, writeLevel0Table");
		    	compactions++;
		...
		System.out.printf("[DEBUG] recoverLogFile, recordCount=%d\n", recordCount);
		
		file.delete(); //should close file
		
		System.out.printf("[DEBUG] recoverLogFile, keep reusing the last log file, status=%s, options.reuseLogs=%s, lastLog=%s, compactions=%d\n",
				status, options.reuseLogs, lastLog, compactions);
		
		// See if we should keep reusing the last log file.
		if (status.ok() && options.reuseLogs && lastLog && compactions == 0) {
			System.out.println("[DEBUG] recoverLogFile 2, writeLevel0Table");
			assert(logFile == null);
		    ...
		    System.out.printf("[DEBUG] recoverLogFile 2, b1=%s, b2=%s\n", b1, b2);
		...
		if (memtable != null)
			System.out.printf("[DEBUG] recoverLogFile, keep reusing the last log file, memtable.entrySize=%d\n",
				memtable.entrySize());
	...
	Status writeLevel0Table(MemTable mem, VersionEdit edit, Version base) {
		...
		Status s = Status.ok0();
		{
			...
			System.out.println("[DEBUG] DBImpl.writeLevel0Table 2, s="+s);
			
		    mutex.lock();
		}
		
		Logger0.log0(options.infoLog, "Level-0 table #{}: {} bytes {}", meta.number, meta.fileSize, s);
		...
		System.out.printf("[DEBUG] writeLevel0Table 1, level=%d\n", level);
	 ...
	 * EXCLUSIVE_LOCKS_REQUIRED(mutex)
	 * 
	 * @param force compact even if there is room
	 * @return
	 */
	...
	void maybeScheduleCompaction() {
		System.out.println("[DEBUG] DBImpl.maybeScheduleCompaction start, manualCompaction="+manualCompaction);
		mutex.assertHeld();
		if (bgCompactionScheduled) {
			...
			System.out.println("[DEBUG] DBImpl.maybeScheduleCompaction: DB is being deleted; no more background compactions");
		} else if (!bgError.ok()) {
		    // Already got an error; no more changes
			System.out.println("[DEBUG] DBImpl.maybeScheduleCompaction: Already got an error; no more changes, bgError="+bgError);
		} else if (immtable == null &&
		             manualCompaction == null &&
		             !versions.needsCompaction()) {
		    // No work to be done
			System.out.println("[DEBUG] DBImpl.maybeScheduleCompaction: No work to be done");
		} else {
			//System.out.println("[DEBUG] DBImpl.maybeScheduleCompaction: env.schedule(new BgWorkRunnable())");
	...
	void backgroundCall() {
		mutex.lock();
		System.out.println("[DEBUG] backgroundCall start");
		try {
	...
	Status openCompactionOutputFile(CompactionState compact) {
		...
		System.out.printf("[DEBUG] =========> openCompactionOutputFile");
	...
	Status installCompactionResults(CompactionState compact) {
		mutex.assertHeld();
		Logger0.log0(options.infoLog,  "Compacted {}@{} + {}@{} files => {} bytes",
	...
	Status doCompactionWork(CompactionState compact) {
		...
		Logger0.log0(options.infoLog,  "Compacting {}@{} + {}@{} files",
			      compact.compaction.numInputFiles(0),
			      compact.compaction.level(),
			      compact.compaction.numInputFiles(1),
			      compact.compaction.level() + 1);
		...
		for (; input.valid() && shuttingDown.get() == null; ) { //shutting_down_.Acquire_Load()
			...
			if (drop) {
				...
				if (compact.builder.fileSize() >= compact.compaction.maxOutputFileSize()) {
					status = finishCompactionOutputFile(compact, input);
					System.out.println("[DEBUG] doCompactionWork 2, status"+status);
					if (!status.ok())
						break;
				}
			}
	...
	public void TEST_CompactRange(int level, Slice begin, Slice end) throws Exception {
		System.out.println("[DEBUG] DBImpl.TEST_CompactRange start");
		...
		System.out.println("[DEBUG] DBImpl.TEST_CompactRange 2");
		...
		try {
			...
			while (!manual.done && shuttingDown.get() == null && bgError.ok()) {
			    if (manualCompaction == null) {  // Idle
			    	...
			    	System.out.println("[DEBUG] DBImpl.TEST_CompactRange, manualCompaction="+manualCompaction);
			    	maybeScheduleCompaction();
