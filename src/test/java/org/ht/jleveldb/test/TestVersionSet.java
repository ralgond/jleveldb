...
import org.ht.jleveldb.db.FileMetaData;
import org.ht.jleveldb.db.VersionSetGlobal;
...
public class TestVersionSet {
	static class FindFileTest {
		...
		public void add(String smallest, String largest,
		    ...
		    f.smallest = new InternalKey(new Slice(smallest), smallest_seq, ValueType.Value);
		    f.largest = new InternalKey(new Slice(largest), largest_seq, ValueType.Value);
		    files.add(f);
		}
		
		public int find(String key) {
		    InternalKey target = new InternalKey(new Slice(key), 100, ValueType.Value);
		...
		public boolean overlaps(String smallest, String largest) {
		    InternalKeyComparator cmp = new InternalKeyComparator(BytewiseComparatorImpl.getInstance());
		    Slice s = new Slice(smallest != null ? new Slice(smallest) : new Slice(""));
		    Slice l = new Slice(largest != null ? new Slice(largest) : new Slice(""));
